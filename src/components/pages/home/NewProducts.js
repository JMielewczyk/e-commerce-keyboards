...
const NewProducts = () => {
    ...
    const loadContent = () => {
        ...
        const categories = Object.keys(data).filter(element => true);
        ...
        const MapProducts = filteredNewProducts.map((element, index) => {
            return (
                <Link style={{backgroundImage: `url(${element.imageTemplate})`}} className='newProduct-container' key={element.name} to={`/home/${matchedCategories[index]}/${element.name}`}>
                    <div className='container'>
                    <div className='background-blur'></div>
                    <div className='product-container' >
                    <p className='product-name'>{element.name}</p></div>
                    </div>
                </Link>
            )
        })
